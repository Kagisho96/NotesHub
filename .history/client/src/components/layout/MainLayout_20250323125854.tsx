...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen flex ${darkMode ? 'dark bg-gray-900' : 'bg-gray-100'}`}>
      ...
      <aside className={`w-64 ${darkMode ? 'bg-gray-800 text-white' : 'bg-primary-700 text-white'} shadow-xl fixed h-full z-30`}>
      ...
      <main className={`flex-1 ${darkMode ? 'bg-gray-800' : 'bg-gray-100'} ml-64`}>
        <header className={`${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'} shadow-md h-16 flex items-center px-6 sticky top-0 z-20`}>
          <div className="flex justify-between items-center w-full">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                className={`${darkMode ? 'text-gray-300 hover:text-white' : 'text-gray-600 hover:text-gray-800'} font-medium shadow-sm px-3 py-1 rounded`}
              >
                Help
              </button>
            </div>
