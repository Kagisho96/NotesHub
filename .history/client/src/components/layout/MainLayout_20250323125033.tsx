...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  return (
    <div className={`min-h-screen flex ${darkMode ? 'dark bg-gray-900' : 'bg-gray-50'}`}>
      {/* Sidebar */}
      <aside className={`w-64 ${darkMode ? 'bg-gray-800 text-white' : 'bg-primary-700 text-white'} shadow-lg z-10`}>
        <div className="p-4">
              ...
              <Link
                ...
                className={`block px-4 py-2 w-full text-left transition ${
                  location.pathname === '/dashboard'
                    ? darkMode ? 'bg-gray-700 font-medium' : 'bg-primary-600 font-medium'
                    : darkMode ? 'hover:bg-gray-700' : 'hover:bg-primary-600'
                }`}
              >
                Dashboard
              </Link>
        ...
        <div className="absolute bottom-0 w-64 p-4">
          <div className="flex items-center gap-3 mb-3">
            <div className={`w-10 h-10 rounded-full ${darkMode ? 'bg-gray-600 text-white' : 'bg-primary-300 text-primary-800'} flex items-center justify-center font-semibold`}>
      ...
      {/* Main Content */}
      <main className={`flex-1 ${darkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
        <header className={`${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'} shadow-sm h-16 flex items-center px-6 sticky top-0 z-10`}>
          <div className="flex justify-between items-center w-full">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className={`p-2 rounded-md ${darkMode ? 'text-yellow-300 hover:bg-gray-700' : 'text-gray-600 hover:bg-gray-200'}`}
